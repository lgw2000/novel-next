...
function buildAdminFilter(filter?: AdminResponseFilter) {
  ...
  if (filter.username) {
    conditions.username = { contains: filter.username };
  }

  if (filter.authorId) {
    conditions.authorId = { contains: filter.authorId };
  }

  if (filter.email) {
    conditions.user = { email: { contains: filter.email } };
  }

  if (filter.ip) {
    conditions.ip = { contains: filter.ip };
  }
...
export const responseRepository: ResponseRepository = {
  ...
  async findRecentByThreadId(
    ...
    // SQLite-compatible: fetch seq=0 and latest N responses separately then merge
    const [seqZero, latestResponses] = await Promise.all([
    ...
    // Merge and sort by seq ascending
    const merged = [...seqZero, ...latestResponses].sort((a, b) => a.seq - b.seq);
    return merged;
