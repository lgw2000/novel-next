...
export default async function ThreadDetailPage({ params, searchParams }: Props) {
  ...
  try {
    const [thread, allBoards, session, settings, _prevThread, _nextThread] = await Promise.all([
      threadService.findById(threadIdNum),
      boardService.findAll(),
      getServerSession(authOptions),
      globalSettingsService.get(),
      prisma.thread.findFirst({
        where: { boardId, id: { lt: threadIdNum }, published: true, deleted: false },
        orderBy: { id: "desc" },
        select: { id: true }
      }),
      prisma.thread.findFirst({
        where: { boardId, id: { gt: threadIdNum }, published: true, deleted: false },
        orderBy: { id: "asc" },
        select: { id: true }
      })
    ]);

    const prevThread = _prevThread as { id: number } | null;
    const nextThread = _nextThread as { id: number } | null;
